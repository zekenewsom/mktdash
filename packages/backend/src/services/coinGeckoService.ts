...
export async function getCryptoDetails(cryptoId: string, vsCurrency: string = 'usd', days: string = 'max') {
  ...
  try {
    const [infoRes, chartRes] = await Promise.all([
      axios.get<CoinGeckoCoinInfoResponse>(`${COINGECKO_BASE_URL}/coins/${cryptoId}?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false`, { headers }),
      axios.get<CoinGeckoMarketChartResponse>(`${COINGECKO_BASE_URL}/coins/${cryptoId}/market_chart?vs_currency=${vsCurrency}&days=${days}&interval=daily`, { headers })
    ]);

    const coinInfo = infoRes.data;
    const historicalPrices = chartRes.data.prices;
